...
func _physics_process(delta):
	...
	if colider != null and colider is ConicalFlask :
		...
		if other_flask.id not in known_ids and is_below(other_flask):
			...
			if id in other_flask.known_ids:
				other_flask.known_ids.erase(id)
			
			if debug_show_id:
				$Label3D2.text = "known ids nuts : " + str(known_ids)
		else :
			if delete_me != other_flask.id:
				print(other_flask.id)
			delete_me = other_flask.id
